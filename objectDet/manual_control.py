...
class World(object):
    ...
    def restart(self):
        ...
        if self.player is not None:
            ...
            self.destroy()
            self.player = self.world.try_tor(blueprint, spawn_point)
            self.show_vehicle_telemetry = False
...
class HUD(object):
    def __init__(self, width, height):
        ...
        # self._ackermann_control = carla.VehiclenControl()
...
# SPAWN NPCs and Actors
def spawn_npc_vehicles(world, blueprint_library, number_of_vehicles=10):
    # Get a list of NPC vehicle blueprints
    npc_blueprints = blueprint_library.filter('vehicle.*.*')

    # Choose random spawn points for NPC vehicles
    spawn_points = random.sample(world.get_map().get_spawn_points(), min(number_of_vehicles, len(world.get_map().get_spawn_points())))

    # Spawn NPC vehicles
    npc_vehicles = []
    for spawn_point in spawn_points:
        blueprint = random.choice(npc_blueprints)
        npc_vehicle = world.spawn_actor(blueprint, spawn_point)
        npc_vehicles.append(npc_vehicle)

    return npc_vehicles

def spawn_other_actors(world, blueprint_library, number_of_actors=5):
    # Get a list of other actor blueprints (e.g., pedestrians)
    actor_blueprints = blueprint_library.filter('walker.pedestrian.*')

    # Choose random spawn points for other actors
    spawn_points = random.sample(world.get_map().get_spawn_points(), min(number_of_actors, len(world.get_map().get_spawn_points())))

    # Spawn other actors
    other_actors = []
    for spawn_point in spawn_points:
        blueprint = random.choice(actor_blueprints)
